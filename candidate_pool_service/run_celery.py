...
For Celery to run from command line, script runs as separate process with celery command
 ...
 Run the following command to start celery worker:
     celery -A candidate_pool_service.run.celery  worker --concurrency=4 --loglevel=info
"""
...
celery.start(argv=['celery', 'worker', '-Ofair', '--without-gossip', '-l', 'info', '-Q', 'celery_stats_scheduler'])
