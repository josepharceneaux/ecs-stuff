...
class TestBaseCampaignEvent(object):
    ...
    def test_link_base_campaign_with_deleted_event(self,event_in_db_second, token_first, base_campaign):
        """
        This tests linking of base campaign with deleted event. This should return a Resource not found.
        """
        ...
        response = send_request(self.HTTP_METHOD, self.URL % (base_campaign['id'], event_in_db_second['id']), token_first)
        assert response.status_code == codes.NOT_FOUND, response.text
