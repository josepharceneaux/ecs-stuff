...
class TalentPoolCandidate(db.Model):
    ...
    def candidates_added_last_month(cls, user_name, talent_pool_name, user_specific_date, user_id):
        ...
        :param datetime|None|unicode user_specific_date: Datetime this should be later than or equal to updated_time
        or added_time
        :rtype: int
        """
        if type(user_specific_date) == datetime:
        ...
        if type(user_specific_date) == unicode:
            return cls.query.filter(cls.talent_pool_id == TalentPool.id) \
