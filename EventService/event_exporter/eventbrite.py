...
from event_exporter.common import _log_error, EventNotCreated, EventLocationNotCreated, TicketsNotCreated, \
    EventNotPublished, EventNotUnpublished, EventInputMissing, _log_exception
...
class Eventbrite(TalentEventBase):
    """
        ...
        if (user_credentials.userId == user_credentials.userId) \
            ...
            if response.ok:
                try:
                    ...
                    self.message_to_log.update({'error': error_message})
                    _log_exception(self.message_to_log)
            else:
