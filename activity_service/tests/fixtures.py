...
def activities_fixture(user_fixture, candidate_source_fixture, domain_fixture, request):
    activities = []
    ...
    activities.append(Activity(added_time=TODAY + timedelta(hours=-2), source_table=User.__tablename__,
                               source_id=1, type=12, user_id=user_fixture.id, domain_id=domain_fixture.id,
    ...
    for i in xrange(3):
        activities.append(
            Activity(added_time=TODAY, source_table=User.__tablename__, source_id=1,
                     type=12, user_id=user_fixture.id, domain_id=domain_fixture.id, params=json.dumps({'lastName': last_name,
    ...
    for i in xrange(7):
        activities.append(
            Activity(added_time=C.CANDIDATE_UPDATE_START + timedelta(days=i),
                     ...
                     user_id=user_fixture.id, domain_id=domain_fixture.id, params=json.dumps(
                         {'id': 1337, 'name': first_name}))
        )
        activities.append(
            Activity(added_time=C.SMARTLIST_DELETE_START + timedelta(days=i),
                     source_table='smartlist', source_id=i,
                     type=9, user_id=user_fixture.id, domain_id=domain_fixture.id, params=json.dumps({'name': 'a SL',
                                                                         'is_smartlist': 1}))
        )
        activities.append(
            Activity(added_time=C.EVENT_CREATE_START + timedelta(days=i),
                     source_table='smartlist', source_id=i,
                     type=28, user_id=user_fixture.id, domain_id=domain_fixture.id, params=json.dumps(
        ...
        activities.append(
            Activity(added_time=C.CAMPAIGN_EMAIL_OPEN_START + timedelta(days=i),
                     ...
                     type=16, user_id=user_fixture.id, domain_id=domain_fixture.id, params=json.dumps(
