...
def get_or_create_areas_of_interest(domain_id, include_child_aois=False):
    ...
    if not domain_id:
        current_app.logger.error("get_or_create_areas_of_interest: domain_id is %s!", domain_id)
    ...
    if not len(areas):
        for description in DEFAULT_AREAS_OF_INTEREST:
            AreaOfInterest(description=description, domainId=domain_id)
            db.session.commit()
        areas = db.session.query(AreaOfInterest).filter_by(domain_id=domain_id).order_by(AreaOfInterest.id.asc())
...
def get_area_of_interest_id_to_sub_areas_of_interest(domain_id):
    ...
    sub_areas_of_interest = db.session.query(AreaOfInterest).filter(AreaOfInterest.domain_id == domain_id,
                                                                    AreaOfInterest.parent_id != None).sorted(
...
def add_aoi_to_candidate(candidate_id, aoi_ids, owner_user_id=None):
    if not isinstance(aoi_ids, list):
        aoi_ids = [aoi_ids]
    from candidate_service.app.views.common_functions import domain_id_from_user_id
    logger = current_app.logger
    owner_id = db.session.query(Candidate).filter(Candidate.id == candidate_id).get(Candidate.user_id)
    ...
    current_candidate_aoi_ids = [r.area_of_interest_id for r in db.session.query(CandidateAreaOfInterest).filter_by(
        CandidateAreaOfInterest.candidate_id == candidate_id).get()]
    for new_aoi_id in aoi_ids:
        ...
        if new_aoi_id not in current_candidate_aoi_ids:
            aoi_row = db.session.query(AreaOfInterest).filter_by(AreaOfInterest.id == new_aoi_id).first()
            # Find the AOI amongst the domain's AOIs
            # aoi_row = domain_areas_of_interest.find(lambda r: r.id == new_aoi_id).first()
            if aoi_row:
                try:
                    CandidateAreaOfInterest(areaOfInterestId=new_aoi_id, candidateId=candidate_id)
                    db.session.commit()
                ...
                if aoi_row.parentId:
                    existing_parent_candidate_aoi = db.session.query(CandidateAreaOfInterest).filter(
                        CandidateAreaOfInterest.area_of_interest_id == aoi_row.parentId,
                        CandidateAreaOfInterest.candidate_id == candidate_id).first()
                    if not existing_parent_candidate_aoi:
                        db.session.add(CandidateAreaOfInterest(area_of_interest_id=aoi_row.parentId,
                                                               candidate_id=candidate_id))
                        current_candidate_aoi_ids.append(aoi_row.parentId)
            else:
                new_aoi_domain_id = db.session.query(AreaOfInterest).filter_by(
                    AreaOfInterest.id == new_aoi_id).get(domain_id)
