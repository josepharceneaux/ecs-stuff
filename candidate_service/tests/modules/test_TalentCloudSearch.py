...
from candidate_service.common.models.misc import CustomField, CustomFieldCategory
...
def populate_candidates(owner_user_id, count=1, first_name=True, last_name=True, added_time=True, objective=False,
    ...
    for i in range(count):
        data = {
            ...
            'military_to_date': {True: datetime.date.today(), False: None}.get(military_to_date, military_to_date),
            'source_id': source_id,
...
def test_owner_facet(domain_id, admin_user, sample_user):
    ...
    _assert_search_results(domain_id, {'usernameFacet': sample_user}, normal_user_candidates, check_for_equality=True, no_wait=True)
    ...
    _assert_search_results(domain_id, {'usernameFacet': [admin_user, sample_user]}, total_candidates, check_for_equality=True, no_wait=True)
...
def test_sort_by_match(domain_id, sample_user):
    ...
    # Reverse order (New vision appeared 3times for candidate3, 2times for candidate 2, and one time for candidate 1.)
...
def test_date_of_separation(domain_id, admin_user):
    ...
    _assert_search_results(domain_id, {'military_end_date_from': 2010}, candidate_ids=candidates_2012+candidates_2014 +
                                                                                      candidates_today, no_wait=True)
...
def _assert_search_results(domain_id, search_vars, candidate_ids, no_wait=False, smart_wait=False, check_for_equality=False,
    ...
    for parent_aoi_name, child_aoi_names in KAISER_PARENT_TO_CHILD_AOIS.items():
        ...
        for child_aoi_name in child_aoi_names:
            ...
            if existing_child_aoi:
                ...
                child_aoi = AreaOfInterest(domain_id=domain_id, description=child_aoi_name,
                                                             parent_id=parent_aoi_id)
