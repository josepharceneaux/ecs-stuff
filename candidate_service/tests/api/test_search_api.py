...
# TODO: fix test - Amir
def to_fix_test_search_source(user_first, access_token_first, talent_pool):
    ...
    candidate_ids = populate_candidates(count=5, source_id=source_id)
...
# def to_fix_test_search_military_date_of_separation(user_first, access_token_first, talent_pool):
#     """
...
#     user_id = user_first.id
#
#     candidates_today = populate_candidates(talent_pool=talent_pool, access_token=access_token_first,
#                                            owner_user_id=user_id, count=5,
#                                            military_to_date=str(datetime.datetime.utcnow().date()),
#                                            military_grade=True, military_status=True, military_branch=True)
#     candidates_2014 = populate_candidates(talent_pool=talent_pool, access_token=access_token_first,
#                                           owner_user_id=user_id, count=3,
#                                           military_to_date='2014-04-26')
#     candidates_2012 = populate_candidates(talent_pool=talent_pool, access_token=access_token_first,
#                                           owner_user_id=user_id, count=2,
...
def test_position_and_company(user_first, access_token_first, talent_pool):
    """
...
def _test_search_by_university(sample_user):
    ...
    university1_candidates = populate_candidates(user_id=sample_user.id, school_name=university1)
    # Create other candidates with other university, check
    university2_candidates = populate_candidates(count=2, user_id=sample_user.id, school_name=university2)
...
def _test_search_by_location(sample_user):
    ...
    city, state, zip_code = random.choice(VARIOUS_US_LOCATIONS)
    candidate_ids = populate_candidates(count=2, user_id=sample_user.id, city=city, state=state,
                                        zip_code=zip_code)
...
def _test_location_with_radius(sample_user):
    ...
    _10_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False, **location_within_10_miles)
    _10_mile_candidate_2 = populate_candidates(user_id=sample_user.id, update_now=False,
                                               **location_within_10_miles_2)
    # 25 mile candidates with city state
    _25_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False, **location_within_25_miles)
    _25_mile_candidate_2 = populate_candidates(user_id=sample_user.id, update_now=False,
                                               **location_within_25_miles_2)
    _50_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False, **location_within_50_miles)
    _75_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False, **location_within_75_miles)
    _100_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False,
                                              **location_within_100_miles)
    # The following candidate will not appear in search with radius
    _more_than_100_mile_candidate = populate_candidates(user_id=sample_user.id, update_now=False,
...
def _test_sort_by_proximity(sample_user):
    ...
    # 10 mile candidates with city & state
    _10_mile_candidate = populate_candidates(user_id=user_id, update_now=False, **location_within_10_miles)
    _10_mile_candidate_2 = populate_candidates(user_id=user_id, update_now=False, **location_within_10_miles_2)
    # 25 mile candiates with city state
    _25_mile_candidate = populate_candidates(user_id=user_id, update_now=False, **location_within_25_miles)
    _50_mile_candidate = populate_candidates(user_id=user_id, update_now=False, **location_within_50_miles)
...
def _to_fix_test_search_by_major(sample_user):
    ...
    major1_candidates = populate_candidates(count=2, user_id=sample_user.id, major=major1,
                                            school_name=True, update_now=False)
    major2_candidates = populate_candidates(count=7, user_id=sample_user.id, major=major2,
                                            school_name=True, update_now=False)
...
def _test_search_by_degree(sample_user):
    ...
    degree1 = 'Masters'
    degree2 = 'Bachelors'
    master_candidates = populate_candidates(count=3, user_id=sample_user.id, degree_title=degree1,
                                            school_name=True, update_now=False)
    bachelor_candidates = populate_candidates(count=4, user_id=sample_user.id, degree_title=degree2,
                                              school_name=True, update_now=False)
...
def to_fix_test_sort_by_added_date(sample_user):
    ...
    candidate3 = populate_candidates(user_id=sample_user.id,
                                     ...
                                     update_now=False)
    # Candidate added today (now)
    candidate4 = populate_candidates(user_id=sample_user.id, added_time=datetime.datetime.now(), update_now=False)
