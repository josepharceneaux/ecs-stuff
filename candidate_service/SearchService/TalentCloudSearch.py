...
from TalentAreasOfInterest import get_or_create_areas_of_interest
...
def define_index_fields():
    ...
    for index_field_name, index_field_options in INDEX_FIELD_NAME_TO_OPTIONS.iteritems():
        current.logger.info("Defining index field %s", index_field_name)
...
def delete_index_fields():
    ...
    for index_field_name in INDEX_FIELD_NAME_TO_OPTIONS:
        current.logger.info("Deleting index field %s", index_field_name)
...
def upload_candidate_documents_in_domain(domain_id):
    ...
    current.logger.info("Uploading %s candidates of domain %s", len(candidate_ids), domain_id)
...
def upload_candidate_documents_of_user(user):
    ...
    candidate_ids = [candidate['id'] for candidate in candidates]
    current.logger.info("Uploading %s candidates of user's domain %s", len(candidate_ids), user.domainId)
...
def delete_all_candidate_documents():
    ...
    while not no_more_candidates:
        ...
        # Now delete them
        current.logger.debug("delete_all_candidate_documents: Deleting %s candidate documents", len(candidate_ids_to_delete))
...
def search_candidates(domainId, vars, search_limit=15, candidate_ids_only=False, get_percentage_match=False, count_only=False):
    """
    Searches candidates based on domainId and Vars provided.
    Search Engine: Amazon Cloud Search

    :param
        domainId: Search candidates in given domain Id
    ...
    #TODO check if we can use weights (^) instead of following logic (http://docs.aws.amazon.com/cloudsearch/latest/developerguide/weighting-fields.html)
    #score boosting sections
    ### if query:
    ###     search_queries.append('position:(%s)' % query)
    ###     search_queries.append('skill_description:(%s)' % query)
    ...
    if location:
        ...
        if coords_dict and coords_dict.get('point'):
            ...
            # If coodinates were not found with given location and if location is type of zipcode, then search based on zipcode.
    ...
    """
    filter_queries.append("(term field=domain_id %s)" % domainId)

    # filter_queries.append('isWebHidden:(false)')
    ...
    if type(vars.get('usernameFacet')) == list:
        ...
        ##filter_queries.append('user_id:(%s)' % vars.get('user_id'))
        filter_queries.append("(term field=user_id %s)"%vars.get('usernameFacet'))
        ##filter_queries_cs.append("(term field=user_id %d)"%vars.get('user_id'))

    # if type(vars.get('usernameFacet')) == list:
    #     filter_queries.append('{!tag=dt}usernameFacet:(%s)' % ' OR '.join( vars.get('usernameFacet') ))
    # elif vars.get('usernameFacet'):
    #     filter_queries.append('{!tag=dt}usernameFacet:(%s)' % vars.get('usernameFacet'))
    ...
    if type(vars.get('areaOfInterestIdFacet')) == list:
        ##filter_queries.append('area_of_interest_id:(%s)' % ' OR '.join(vars.get('areaOfInterestIdFacet')))
        ##filter_queries.append("(or term field=area_of_interest_id %s)" % " ".join(vars.get('areaOfInterestIdFacet')))
    ...
    # areaOfInterestNameFacet is no more used so commenting it.
    # if vars.get('areaOfInterestNameFacet') or vars.get('area_of_interest_id'):
    #
    #     # Get all AOI IDs that match areaOfInterestNameFacet for backwards compatibility
    #     if vars.get('areaOfInterestNameFacet'):
    #         area_of_interest_names = vars.get('areaOfInterestNameFacet')
    #         if not type(area_of_interest_names) == list:
    #             area_of_interest_names = [area_of_interest_names]
    #
    #         all_areas_of_interest = db(db.area_of_interest.domainId == domainId).select()
    #
    #         # Find all area_of_interest rows that match the input AOI names
    #         matching_areas_of_interest = all_areas_of_interest.find(
    #             lambda area_of_interest_row: any(filter(lambda name: area_of_interest_row.description in name, area_of_interest_names))
    #         )
    #
    #         area_of_interest_ids = [area_of_interest.id for area_of_interest in matching_areas_of_interest]
    #
    #     # Using areaOfInterestIdFacet is the new, preferred way
    #     else:
    #         if type(vars.get('area_of_interest_id')) == list:
    #             area_of_interest_ids = [str(aoi_id) for aoi_id in vars.get('area_of_interest_id')]
    #         else:
    #             area_of_interest_ids = [str(vars.get('area_of_interest_id'))]
    #
    #     filter_queries.append('{!tag=areaOfInterest}area_of_interest_id:(%s)' % (' OR '.join(str(i) for i in area_of_interest_ids)))

    # if type(vars.get('areaOfInterestNameFacet')) == list:
    #     #search for exact values in facets
    #     area_facets = [ '"%s"' % area_name_facet for area_name_facet in vars.get('areaOfInterestNameFacet') ]
    #     area_of_interests = db(db.area_of_interest.domainId == domainId).select()
    #     area_of_interest_ids = [area_of_interest.id for area_of_interest in area_of_interests]
    #     # filter_queries.append('{!tag=areaOfInterest}areaOfInterestNameFacet:(%s)' % ' OR '.join( area_facets ))
    # elif vars.get('areaOfInterestNameFacet'):
    #     # filter_queries.append('{!tag=areaOfInterest}areaOfInterestNameFacet:("%s")' % vars.get('areaOfInterestNameFacet'))

    if type(vars.get('statusFacet')) == list:
        #search for exact values in facets
        ##status_facets = [ "'%s'" % status_facet for status_facet in vars.get('statusFacet') ]
    ...
    if type(vars.get('positionFacet')) == list:
        ...
        ##filter_queries.append('position:(%s)' % ' AND '.join( position_facets ))
    ...
    if type(vars.get('skillDescriptionFacet')) == list:
        ...
        ##filter_queries.append('skill_description:(%s)' % ' AND '.join( skill_facets ))
    ...
    if type(vars.get('degreeTypeFacet')) == list:
        ...
        ##filter_queries.append('degree_type:(%s)' % ' OR '.join( degree_facets ))
        ...
        ##filter_queries_cs.append("(term field=degree_type '%s')"%vars.get('degreeTypeFacet'))
    ...
    if type(vars.get('schoolNameFacet')) == list:
        ...
        ##filter_queries.append('school_name:(%s)' % ' OR '.join( school_facets ))
    ...
    if type(vars.get('concentrationTypeFacet')) == list:
        ...
        ##filter_queries.append('concentration_type:(%s)' % ' AND '.join( concentration_facets ))
    ...
    if type(vars.get('serviceStatus')) == list:
        ...
        ##filter_queries.append('military_service_status:(%s)' % ' OR '.join( service_status_facets ))
    ...
    if type(vars.get('branch')) == list:
        ...
        ##filter_queries.append('military_branch:(%s)' % ' OR '.join( branch_facets ))
    ...
    # kaiser_custom_cities_list = list()   # TODO we should actually use some Google geocoding/maps API to verify the City of Interest & State of Interest values. We should then re-implement the filtering using kaiser_custom_cities_list. We could also use their Places Autocomplete API on the frontend, instead.
    ...
    if len(custom_field_vars):
        ...
        if len(city_of_interest_custom_fields):
            city_of_interest_custom_fields_facets = ["custom_field_id_and_value:%s" % city_of_interest_custom_fields_facet for city_of_interest_custom_fields_facet in city_of_interest_custom_fields]
    ...
    ##anded_q = ''
    ##if vars.get('id'):
        ##filter_queries.append("(id %s)" % vars['id'])
        # anded_q = "%s AND " % vars['id']

    # Perform search
    # qf="position^1.5 skillDescription^1.2",
    # defType="edismax"
    #params = dict(q=' OR '.join(search_queries), fq=filter_queries, sort=sort, start=offset, size=search_limit)
    ### @CHECK  need to check alternative for filter_queries value. currently it is giving fq= ['domain_id:(109)']
    ### which is not acceptable by cloud search, need to find proper resolution for [ sign
    ...
    # If the NUID custom field exists for domain (kaiser), create a boolean facet of whether each candidate has a NUID or not
    #Commenting below code as in prod also it is not working. Later, we have to figure out solution for this.
    ##if current.IS_DEV and domainId == 90:
    ##    nuid_custom_field = db((db.custom_field.domainId == domainId) & (db.custom_field.id == 14)).select()
    ##    if len(nuid_custom_field):
    ##        params['facet.query'] = ['{!ex=nuid key="Has NUID"}custom_field_id_and_value:14|*']

    # params['facet.field'] = ['{!ex=degree}degree_type', 'skill_description',
    #                          '{!ex=school}school_name', 'concentration_type',
    #                          '{!ex=branch}branch', '{!ex=grade}highest_grade', '{!ex=serviceStatus}service_status',
    #                          '{!ex=source}source_id', '{!ex=areaOfInterest}area_of_interest_id',
    #                          '{!ex=status}status_id', 'product_id', '{!ex=position}position',
    #                          '{!ex=dt}user_id', '{!ex=cf}custom_field_id_and_value']
    ### Adding facet fields parameters

    if not count_only:
        params['facet'] = "{area_of_interest_id:{size:500},source_id:{size:50},user_id:{size:50},status_id:{size:50},skill_description:{size:500}," \
                          "position:{size:50},school_name:{size:500},degree_type:{size:50},concentration_type:{size:50},military_service_status:{size:50}," \
                          "military_branch:{size:50},military_highest_grade:{size:50},custom_field_id_and_value:{size:1000}}"
    ...
    ### benchmark on my local machine: with/out
    ### --> CloudSearch search: Took 2.2300260067 with params={'sort': '_score desc', 'fq': '(and (term field=domain_id 1) (term field=source_id 3))', 'return': '_no_fields,_score', 'q.parser': 'lucene', 'start': 0, 'q': '*:*', 'size': 0}
    ### --> CloudSearch search: Took 2.64228987694 with params={'sort': '_score desc', 'fq': '(and (term field=domain_id 1) (term field=source_id 3))', 'return': '_all_fields,_score', 'facet': '{area_of_interest_id: {size:3000},source_id:{size:50},user_id:{size:50},status_id:{size:50},skill_description:{size:500},position:{size:50},school_name:{size:500},degree_type:{size:50},concentration_type:{size:50},military_service_status:{size:50},military_branch:{size:50},military_highest_grade:{size:50},custom_field_id_and_value:{size:1000}}', 'q.parser': 'lucene', 'start': 0, 'q': '*:*', 'size': 0}
    if count_only:
    ...
    try:
        ...
        current.logger.exception("Received exception in search_candidates: search_candidates(%(domainId)s, %(vars)s, %(search_limit)s, %(candidate_ids_only)s, %(get_percentage_match)s, %(count_only)s). params=%(params)s" % locals())
        return context_data
    # import logging
    # from logging.handlers import SMTPHandler
    # app.logger.addHandler("CloudSearch search: Took %s with params=%s", time.time() - start_time, params)

    matches = results['hits']['hit']

    #facets = results.get('facet_counts', dict()).get('facet_fields', dict()) ## its after few lines, below
    ...
    facets = get_faceting_information(results.get('facets'), domainId)

    #Update facets
    _update_facet_counts(filter_queries,params['filter_query'], facets, query_string, domainId)
    ...
    if len(facets.get('custom_field_id_and_value',dict())):
        ...
        for cf_facet_row in facets['custom_field_id_and_value']:
            ...
            # if len(city_of_interest_custom_fields) and int(cf_id) == 39:
            #     if cf_value not in kaiser_custom_cities_list:
            #         continue
    ...
    ##    percentage_matches = [ int((match['score'] / results.maxScore) * 100) for match in matches ]
        # for match in matches:
        #     if match['fields']['_score'] > max_score:
        #         max_score = match['fields']['_score']
    ##else:
    ##    percentage_matches = []
    percentage_matches = []

    # if self._session:
    #     self._session.candidate_id_results = candidate_ids
...
def get_faceting_information(facets, domain_id):
    ...
    if facet_aoi:
        ...
        # for bucket in facet_aoi:
        #     #aoi = db(db.area_of_interest.id == bucket['value']).select().first()
        #     aoi = areas.find(lambda row: row.id == int(bucket['value'])).first()
        #     tmp_dict[aoi.description] = bucket['count']
    ...
    if facet_source:
        pass
        stmt = select([cache.cached(timeout=120)]).where(c_source.c.domainId == domain_id)
        ...
        search_facets_values['sourceFacet'] = get_facet_info_with_ids(facet_source,'description',domain_candidate_source)

    if facet_status:
        pass
        # #tmp_dict = {}
        # candidate_status = db(db.candidate_status).select(cache=(cache.ram, 3000))
        # #for bucket in facet_status:
        # #    c_status = candidate_status.find(lambda row: row.id == int(bucket['value'])).first()
        # #    tmp_dict[c_status.description] = bucket['count']
...
def get_facet_info_with_ids(facet, field_name, rows):
    ...
    #
    #     #tmp_dict[(row.description, bucket['value'])] = bucket['count']
    #     #tmp_dict[row.description] = bucket['count']
    return tmp_dict
...
def _cloud_search_fetch_all(params):
    ...
    while not no_more_candidates:
        ...
        #     current.logger.error("Received error making search request. Search params: %s\nResponse: %s", params, response.text)
        #     error = True
...
def _convert_date_range_in_cloudsearch_format(from_date, to_date):
    ...
    if from_date:
        try:
            ...
            current.logger.exception("Received invalid degree_end_year_from: %s", from_date)
    else:
    ...
    if to_date:
        try:
            ...
            current.logger.exception("Received invalid degree_end_year_to: %s", to_date)
    else:
