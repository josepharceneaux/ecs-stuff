...
printf "\n"
execution_time=$( { time batch_one=`py.test -n 48 user_service/tests activity_service/tests spreadsheet_import_service/tests candidate_pool_service/tests push_campaign_service/tests`; } 2>&1 )
printf "\n========================== Batch 1 execution ends. Took ${execution_time} =========================="
printf "\n========================== Batch 2 execution starts =========================="
printf "\nCandidate Service\nAuth Service\nScheduler Service\nResume Parsing Service"
printf "\n"
execution_time=$( { time batch_two=`py.test -n 48 candidate_service/tests auth_service/tests scheduler_service/tests resume_parsing_service/tests`; } 2>&1 )
...
if [[ "$batch_one" =~ [0-9]+\ passed\ in\ [0-9]*.[0-9]+\ seconds ]]; then batch_one_status=1; else batch_one_status=0 && "$batch_one"; fi
if [[ "$batch_two" =~ [0-9]+\ passed\ in\ [0-9]*.[0-9]+\ seconds ]]; then batch_two_status=1; else batch_two_status=0 && "$batch_two"; fi
if [[ "$batch_three" =~ [0-9]+\ passed\ in\ [0-9]*.[0-9]+\ seconds ]]; then batch_three_status=1; else batch_three_status=0 && "$batch_three"; fi
