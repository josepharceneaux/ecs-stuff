...
def test_user_service_get(access_token, admin_access_token, domain_admin_access_token, ordinary_user, admin_user,
                          domain_admin_user):

    # Get info of ordinary user when logged-in user is DOMAIN_ADMIN
    assert user_api(domain_admin_access_token, ordinary_user.id) == ordinary_user.id

    # Ordinary user getting his own info
    assert user_api(access_token, ordinary_user.id) == ordinary_user.id
    ...
    assert user_api(domain_admin_access_token) == [ordinary_user.id, domain_admin_user.id]


# Test DELETE operation of user API
def test_user_service_delete(access_token, admin_access_token, domain_admin_access_token, ordinary_user, admin_user,
                          domain_admin_user):

    # Non-admin user trying to delete some user
    assert user_api(access_token, ordinary_user.id, action='DELETE') == 401
    ...
    assert user_api(domain_admin_access_token, ordinary_user.id, action='DELETE') == 400
...
def test_user_service_put(access_token, admin_access_token, domain_admin_access_token, ordinary_user, admin_user,
                          domain_admin_user):

    data = {'first_name': gen_salt(6), 'last_name': gen_salt(6), 'phone': '+1 226-581-1027', 'email': ''}

    # ADMIN user trying to update non-existing user
    assert user_api(domain_admin_access_token, ordinary_user.id + 100, data=data, action='PUT') == 404
    ...
    assert user_api(domain_admin_access_token, ordinary_user.id, data=data, action='PUT') == 400
    ...
    assert user_api(domain_admin_access_token, ordinary_user.id, data=data, action='PUT') == ordinary_user.id

    # Refresh ordinary user
    db.session.refresh(ordinary_user)
    db.session.commit()

    # Check if ordinary user has been updated successfully
    assert ordinary_user.email == data['email']
    assert ordinary_user.first_name == data['first_name']
    assert ordinary_user.last_name == data['last_name']
    assert ordinary_user.phone == data['phone']


# Test POST operation of user API
def test_user_service_post(access_token, admin_access_token, domain_admin_access_token, ordinary_user):

    first_user = {
        'first_name': gen_salt(6),
        'last_name': gen_salt(6),
        'phone': '+1 226-581-1027',
        'domain': ordinary_user.domain_id,
        'is_admin': '1'
    }
    ...
    data['users'][1]['email'] = ordinary_user.email
