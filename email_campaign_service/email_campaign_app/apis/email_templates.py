...
class EmailTemplates(Resource):
    ...
    def post(self):
        ...
        if existing_template:
            raise InvalidUsage('Email template with name=%s already exists' % existing_template)
        ...
        if template_folder_id and not (isinstance(template_folder_id, int) or template_folder_id.isdigit()):
            raise InvalidUsage('Invalid input: folder_id must be positive integer')
        ...
        if template_folder and template_folder.domain_id != domain_id:
            raise ForbiddenError("Email template's folder (id:%d) is not in the user's domain (id:%d)."
                                 % (template_folder.id, template_folder.domain_id))

        # Check if email template folder exists with given template_folder_id
        if template_folder_id and not template_folder:
            raise ResourceNotFound("Email template's folder (id:%s) not found." % template_folder_id)
