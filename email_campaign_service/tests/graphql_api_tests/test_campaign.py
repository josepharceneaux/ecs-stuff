...
class TestQueryCampaign(object):
    ...
    def test_get_campaign_in_same_domain(self, access_token_first, email_campaign_of_user_second):
        ...
        """
        query = {'query': self.query['query'] % email_campaign_of_user_second.id}
        response = send_request('get', GRAPHQL_BASE_URL, access_token_first, data=query)
        assert response.status_code == requests.codes.ok
        assert 'errors' not in response.json()
        campaign = response.json()['data']['email_campaign_query']['campaign']
    ...
    def test_get_campaign_from_other_domain(self, access_token_other, email_campaign_of_user_first):
        """
