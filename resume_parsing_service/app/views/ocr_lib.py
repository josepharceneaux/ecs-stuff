...
def ocr_image(img_file_obj):
    API_KEY = 'PDMXB3076888A'
    ...
    try:
        ocr_response = requests.post('https://apipro1.ocr.space/parse/image',
                          files={'demo.png': img_file_obj},
                          data=payload,
                          )
    ...
    return ''.join(result.get('ParsedText') for result in json_ocr_response.get('ParsedResults'))
    # try:
    #     doc_content = doc_content.encode('utf8', 'replace')
    #     encoded_resume = base64.b64encode(doc_content)
    # except Exception as e:
    #     print 'Coudlnt encode'
    #     print e.message
    # optic_response = fetch_optic_response(encoded_resume, filename)
