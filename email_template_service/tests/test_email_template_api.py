...
def get_template_folder(auth_token):
    """

    :param auth_token:
    :return:
    """
    template_folder_name = 'test_template_folder_for_immutable_as_user_manager_%i' % time.time()

    data = {'name': template_folder_name}
    response = requests.post(
        url=EMAIL_TEMPLATE_FOLDER_URI, data=json.dumps(data),
...
class TestCreateImmutableTemplate:
    """
    Test creating immutable template
    """

    def test_create_campaign_template_immutable(self, sample_user, user_auth, email_template_body):
        """
